...
export default function Phone({ grid, cursor, sendJsonMessage, newPixel }) {
	...
	const FullLobbyModal = (
		<div className="sm:hidden absolute screen-dvh z-10 flex place-items-center">
			<FullLobby />
		</div>
	);
	...
	const steps = [
		{
			element: "#canvas",
			intro: `Repère toi sur l'écran en cherchant le curseur ${
				cursor.id != -1 ? colors[cursor.id].name : "noir"
			} ! Utilise la grille pour dessiner sur l'écran. Clique pour colorier. Clique long pour te déplacer.`,
			position: "right",
		},
		{
			element: "#palette",
		...
		{
			element: "#joystick",
			intro: `Le joystick te permet de te déplacer. Tu peux aussi y retrouver la couleur de ton curseur, le ${
				cursor.id != -1 ? colors[cursor.id].name : "noir"
			}.`,
			position: "top",
		},
	...
	return (
		<>
			...
			<div
				className={`sm:hidden ${cursor?.id == -1 ? "opacity-60" : ""}`}
			>
				<Steps
					enabled={enabled}
					steps={steps}
				...
				<div className="flex flex-col screen-dvh bg-[#454141] justify-start">
					...
					>
						<PhoneCanvas
							grid={grid}
							newPixel={newPixel}
							squareSide={windowSize.current[0] / 5}
							cursor={cursor}
					...
					<div
						className="flex my-2 bg-[#686060] rounded-2xl"
						id="palette"
					>
					...
					<div className="relative bg-[#686060] w-full flex flex-col items-center justify-evenly flex-grow rounded-t-2xl">
						<button
							...
							className="absolute top-0 left-0 text-white p-2"
						>
